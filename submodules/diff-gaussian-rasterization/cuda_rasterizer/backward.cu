...
__device__ void computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec3* dL_dcolor, glm::vec3* dL_dmeans, glm::vec3* dL_dshs, float* dL_dcampos)
{
	...
	// Camera-center gradient from view-dependent SH color.
	// dir_orig = mean - campos, so dL/dcampos = -dL/ddir_orig.
	// This is a small but useful part of camera extrinsic gradients when SHs are used.
	if (dL_dcampos)
	{
	...
	bool antialiasing)
{
	...
	// Optional partial camera/view-matrix gradient. This captures the dependency
	// through the transformed mean t used for covariance/depth. It intentionally
	// does not yet include every direct covariance orientation term through W,
	// but it gives a useful camera-pose gradient path while keeping the patch
	// targeted and buildable. Matrix layout matches transformPoint4x3.
	if (dL_dviewmatrix)
	{
		atomicAdd(&dL_dviewmatrix[0],  dL_dtx * mean.x);
		atomicAdd(&dL_dviewmatrix[4],  dL_dtx * mean.y);
		atomicAdd(&dL_dviewmatrix[8],  dL_dtx * mean.z);
		atomicAdd(&dL_dviewmatrix[12], dL_dtx);

		atomicAdd(&dL_dviewmatrix[1],  dL_dty * mean.x);
		atomicAdd(&dL_dviewmatrix[5],  dL_dty * mean.y);
		atomicAdd(&dL_dviewmatrix[9],  dL_dty * mean.z);
		atomicAdd(&dL_dviewmatrix[13], dL_dty);

		atomicAdd(&dL_dviewmatrix[2],  dL_dtz * mean.x);
		atomicAdd(&dL_dviewmatrix[6],  dL_dtz * mean.y);
		atomicAdd(&dL_dviewmatrix[10], dL_dtz * mean.z);
		atomicAdd(&dL_dviewmatrix[14], dL_dtz);
	}
	...
	float* dL_dopacity)
{
	...
	// Optional projection/full-projection matrix gradient from the 2D-mean path.
	// Forward: h = proj * [x,y,z,1], ndc_xy = h_xy / h_w.
	// The rasterizer stores dL_dmean2D in normalized device coordinates.
	if (dL_dprojmatrix)
	{
		const float gx = dL_dmean2D[idx].x;
		const float gy = dL_dmean2D[idx].y;
		const float inv_w = m_w;
		const float inv_w2 = inv_w * inv_w;
		const float dL_dhx = gx * inv_w;
		const float dL_dhy = gy * inv_w;
		const float dL_dhw = -(gx * m_hom.x + gy * m_hom.y) * inv_w2;

		atomicAdd(&dL_dprojmatrix[0],  dL_dhx * m.x);
		atomicAdd(&dL_dprojmatrix[4],  dL_dhx * m.y);
		atomicAdd(&dL_dprojmatrix[8],  dL_dhx * m.z);
		atomicAdd(&dL_dprojmatrix[12], dL_dhx);

		atomicAdd(&dL_dprojmatrix[1],  dL_dhy * m.x);
		atomicAdd(&dL_dprojmatrix[5],  dL_dhy * m.y);
		atomicAdd(&dL_dprojmatrix[9],  dL_dhy * m.z);
		atomicAdd(&dL_dprojmatrix[13], dL_dhy);

		atomicAdd(&dL_dprojmatrix[3],  dL_dhw * m.x);
		atomicAdd(&dL_dprojmatrix[7],  dL_dhw * m.y);
		atomicAdd(&dL_dprojmatrix[11], dL_dhw * m.z);
		atomicAdd(&dL_dprojmatrix[15], dL_dhw);
	}
