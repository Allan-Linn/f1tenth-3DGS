	...
	const bool debug)
{
  ...
  // Experimental camera-gradient outputs. These are partial gradients for the
  // view matrix, full projection matrix, and camera center. They are returned
  // by the CUDA extension so the Python training code can later wire them into
  // learnable camera-pose parameters.
